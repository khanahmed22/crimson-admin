...
const Map = forwardRef(function Map(
  ...
  useEffect(() => {
    ...
    const loadHandler = () => {
      ...
      // Apply projection after load (optional)
      if (projection) {
  ...
  // Handle style prop change (light-only)
  useEffect(() => {
    if (!mapInstance) return;
    if (!style) return;

    // If style changes, update it
...
function MapMarker({
  ...
  // Sync other marker options safely
...
function MarkerPopup({
  ...
  return createPortal(
    <div
      className={cn(
        "relative animate-in fade-in-0 zoom-in-95 rounded-md border bg-popover p-3 text-popover-foreground shadow-md",
        className
      )}
    >
      {closeButton && (
...
function MapClusterLayer({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      map.off("mouseenter", unclusteredLayerId, handleMouseLeavePoint);
